...
// Updated handles to match resume
const SOCIAL_LINKS = [
  {
...
const HeroSection = () => {
  return (
    <section
      id="home"
      // FIX: Added pt-24 to push content below the fixed navbar (h-16 = 64px + buffer).
      // Also tightened horizontal padding on mobile.
      className="min-h-screen pt-24 pb-16 flex items-center justify-center relative overflow-hidden px-5 sm:px-8 md:px-12"
    >
      {/* Background Glow Effects — kept small on mobile to avoid overflow */}
      ...
      <div className="max-w-6xl mx-auto w-full flex flex-col gap-4 items-center text-center z-10">

        {/* Availability Badge */}
        <motion.div
          {...fadeUp(0)}
          className="flex items-center gap-2 sm:gap-3 bg-[#13131a] border border-[#8b5cf6]/30 rounded-full px-4 sm:px-5 py-2 sm:py-2.5 mb-6 sm:mb-10"
        >
        ...
        <motion.h1
          {...fadeUp(0.1)}
          className="text-3xl sm:text-5xl md:text-7xl font-extrabold leading-tight mb-6 sm:mb-8 text-white"
        >
          Hi, I&apos;m{' '}
        ...
        <motion.p
          {...fadeUp(0.2)}
          className="text-base sm:text-xl md:text-2xl text-[#94a3b8] font-medium mb-6 sm:mb-10 max-w-2xl leading-relaxed"
        >
        ...
        <motion.p
          {...fadeUp(0.3)}
          className="text-xs sm:text-sm text-[#a78bfa] font-medium mb-6 sm:mb-10 tracking-widest uppercase leading-relaxed"
        >
        ...
        <motion.div
          {...fadeUp(0.4)}
          className="flex flex-col sm:flex-row gap-4 sm:gap-5 mb-10 sm:mb-16 w-full sm:w-auto"
        >
          <Link
            to="projects"
            smooth
      ...
      {/* Scroll Down Indicator — FIX: Moved OUTSIDE the flex column and truly
          hidden on mobile with 'hidden sm:flex'. Was inside the column before
          which caused it to appear between content items on small screens. */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
