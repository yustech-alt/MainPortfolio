...
const projects = [
  ...
  },
  {
    ...
    live: 'https://project-six.vercel.app',
    featured: false,
  },
]
...
const ProjectCard = ({ project, index }) => (
  ...
  >
    <div className="bg-[#0f0f17] rounded-2xl p-8 h-full flex flex-col gap-5">
      ...
      <p className="text-[15px] text-[#94a3b8] leading-relaxed flex-1">{project.desc}</p>
      <div className="flex flex-wrap gap-2">
        {project.tags.map(tag => (
          <span key={tag} className="text-xs bg-[#8b5cf6]/10 text-[#8b5cf6] border border-[#8b5cf6]/20 px-4 py-1.5 rounded-full">
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className="max-w-6xl mx-auto w-full relative z-10 flex flex-col items-center">
        ...
        >
          ...
          <p className="text-[#94a3b8] mt-8 max-w-xl mx-auto text-sm leading-relaxed">
            Here are some of the projects I&apos;ve worked on. Each one was an opportunity
            to learn something new and solve real problems.
          </p>
        </motion.div>
            ...
            >
              <div className="bg-[#0f0f17] rounded-3xl p-8 flex flex-col sm:flex-row gap-8 items-start sm:items-center">
                ...
                <div className="flex-1 flex flex-col gap-4">
                  ...
                  {/* FIX: Removed redundant text-sm — kept text-[15px] only */}
                  <p className="text-[15px] text-[#94a3b8] leading-relaxed">{project.desc}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="text-xs bg-[#8b5cf6]/10 text-[#8b5cf6] border border-[#8b5cf6]/20 px-4 py-1.5 rounded-full">
                        {tag}
                ...
                {/* Links */}
                <div className="flex gap-3 shrink-0">
                  <a href={project.github} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} on GitHub`} className="w-11 h-11 rounded-xl bg-[#8b5cf6]/10 hover:bg-[#8b5cf6] flex items-center justify-center text-[#94a3b8] hover:text-white transition-all duration-300 text-lg">
                    <FaGithub />
                  </a>
                  <a href={project.live} target="_blank" rel="noopener noreferrer" aria-label={`View ${project.title} live demo`} className="w-11 h-11 rounded-xl bg-[#8b5cf6]/10 hover:bg-[#8b5cf6] flex items-center justify-center text-[#94a3b8] hover:text-white transition-all duration-300 text-lg">
                    <FaExternalLinkAlt />
                  </a>
                </div>
              </div>
